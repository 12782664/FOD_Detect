...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        #avg_out = torch.mean(x, dim=1, keekdim=True)
        avg_out = torch.mean(x, 1, True)
        #avg_out = torch.mean(x, dim=1, keekdim=True, *, dtype=None, out=None)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True, norm_layer=None):
        ...
        # 网络的第一层加入注意力机制 added 2023-11-29
        #self.ca = ChannelAttention(64)
        #self.sa = SpatialAttention()

        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        # self.conv1_2 = nn.Conv2d(self.in_channel, self.in_channel, kernel_size=3, stride=2, padding=1, bias=False)
        # self.bn1_2 = norm_layer(self.in_channel)
        # self.relu2 = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 64, blocks_num[0],sppf=True)
        self.layer2 = self._make_layer(block, 128, blocks_num[1], stride=2)
        self.layer3 = self._make_layer(block, 256, blocks_num[2], stride=2)
        self.layer4 = self._make_layer(block, 512, blocks_num[3], stride=2)

        #网络的卷积层的最后一层加入注意力机制 added 2023-11-29
        #self.ca1 = ChannelAttention(64)
        #self.sa1 = SpatialAttention()

        # 网络的最后层加入注意力机制 added 2024-1-2
    ...
    def _make_layer(self, block, channel, block_num, stride=1,sppf=False):
        ...
        if sppf == True:
            layers.append(SPPF(c1=256,c2=256))
